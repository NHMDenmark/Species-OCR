...
html_output_file_success = env_config.get('HTML_REPORT_SUCCESS')
html_output_file_error = env_config.get('HTML_REPORT_ERROR')
...
for list in [list_success, list_error]:
    for index, group in enumerate(list):
        ...
        success = group['cover']['gbif_match'] is not None and group['cover']['error'] is False
