...
def find_cover_label(img: cv2.Mat, debug: bool = False) -> tuple[cv2.Mat, bool]:
    ...
    for i, contour in enumerate(contours):
        ...
        if len(approx) == 4:  # Approximate polygon is rectangle
            ...
            a = width * height
            if a > 2000000 and hierarchy[0][i][2] > 0:  # Meets area requirement and is a closed rectangle
                candidate_rectangles.append((rect, a))
                cv2.drawContours(img_with_candidate_rectangles, [np.int0(cv2.boxPoints(rect))], 0, (0, 0, 255), 6)
    if debug:
    ...
    if candidate_rectangles.__len__() == 0:
        ...
        winner = candidate_rectangles[np.argmin(np.array([x[1] for x in candidate_rectangles]))][0]

        if debug:
            img_with_contour = img_bottom_left.copy()
            cv2.drawContours(img_with_contour, [np.int0(cv2.boxPoints(winner))], 0, (0, 0, 255), 6)
