...
def inside_rect(rect, num_cols, num_rows):
    # Determine if the four corners of the rectangle are inside the rectangle with width
    # and height
    # rect tuple
    # center (x,y), (width, height), angle of rotation (to the row)
    # center  The rectangle mass center.
    # center tuple (x, y): x is regarding to the width (number of columns) of the image,
    # y is regarding to the height (number of rows) of the image.
...
def rect_bbx(rect):
    ...
    # bounding box for this rotated rectangle, height 4*sqrt(2), width 4*sqrt(2), center
    # (10, 10), angle 0 degree
...
def crop_rectangle(image, rect):
    ...
    return image[
        rect_center_y - rect_height // 2 : rect_center_y
        + rect_height
        - rect_height // 2,
...
def crop_rotated_rectangle(image, rect, shrink_border=0):
    ...
    return rotated_rect_bbx_upright_image[
        int(crop_center[1] - rect_height // 2 + shrink_border) : int(
            crop_center[1] + (rect_height - rect_height // 2) - shrink_border
        ),
        int(crop_center[0] - rect_width // 2 + shrink_border) : int(
            crop_center[0] + (rect_width - rect_width // 2 - shrink_border)
        ),
    ]
